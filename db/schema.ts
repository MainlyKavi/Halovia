/**
 * D1 schema used by the Worker and mirrored in drizzle/0001_halovia_mvp.sql.
 * Statements remain separate because D1 prepare() accepts one statement only.
 */
export const SCHEMA_STATEMENTS = [
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS one_active_journey_per_owner_idx ON journeys(owner_id) WHERE status = 'active'`,
  `CREATE INDEX IF NOT EXISTS journeys_owner_updated_idx ON journeys(owner_id, updated_at DESC)`,
  ...
  `CREATE INDEX IF NOT EXISTS journey_locations_latest_idx ON journey_locations(journey_id, server_received_at DESC)`,
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS journey_events_idempotency_idx ON journey_events(journey_id, idempotency_key) WHERE idempotency_key IS NOT NULL`,
  `CREATE INDEX IF NOT EXISTS journey_events_timeline_idx ON journey_events(journey_id, created_at DESC)`,
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS one_pending_safety_check_idx ON safety_checks(journey_id) WHERE status = 'pending'`,
  ...
  `CREATE INDEX IF NOT EXISTS viewer_sessions_journey_idx ON viewer_sessions(journey_id, created_at DESC)`,
