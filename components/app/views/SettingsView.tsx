...
import { BellRing, ChevronRight, Clock3, Globe2, HeartHandshake, Info, Languages, LocateFixed, MoonStar, Palette, ShieldCheck, Trash2, UserRound, UsersRound, Volume2 } from "lucide-react";
...
import { formatCountry, formatNumber } from "@/lib/i18n/format";
...
export function SettingsView() {
  ...
  async function save() {
    ...
    const emergencyNumber = number.trim() || state.emergency.number;
    try {
      await haloviaApi.updateProfile({ displayName, language: state.user.language, locale: state.user.locale, country: state.user.country, dateFormat: state.user.dateFormat, emergencyNumber, retentionDays: state.privacy.historyRetention === "7" ? 7 : 30, onboardingComplete: state.user.onboardingComplete });
      updateState((current) => ({ ...current, user: { ...current.user, name: displayName }, emergency: { ...current.emergency, number: emergencyNumber } }));
  ...
  return <div className="view-stack settings-view">
    <header className="view-heading"><div><p className="eyebrow"><HeartHandshake size={15} />Halovia</p><h1>{t("settings.title")}</h1><p>{t("settings.subtitle")}</p></div><Button onClick={() => void save()}>{t("common.save")}</Button></header>
    <div className="settings-grid">
      ...
      <Card className="settings-card"><h2><UsersRound size={20} />{t("settings.contacts")}</h2><Link href="/circle" className="settings-link"><span>{formatNumber(state.contacts.filter((contact) => contact.active).length, state.user.locale)} {t("common.active")}</span><ChevronRight size={18} /></Link></Card>
      <Card className="settings-card"><h2><ShieldCheck size={20} />{t("settings.data")}</h2><p className="field-hint">{t("settings.storageDisclosure")}</p><Button variant="danger" onClick={() => setClearOpen(true)}><Trash2 size={17} />{t("settings.clearData")}</Button></Card>
