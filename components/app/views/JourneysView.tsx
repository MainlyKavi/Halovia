...
export function JourneysView() {
  ...
  return <div className="view-stack">
    ...
    {state.history.length === 0 ? <Card><EmptyState icon={<History size={28} />} title={t("journeys.emptyTitle")} text={t("journeys.emptyText")} action={<Link href="/start" className="button button-primary button-md">{t("home.startJourney")}</Link>} /></Card> : <div className="history-list">{state.history.map((journey) => <Link href={`/journeys/${journey.id}`} key={journey.id} className="history-card"><span className="journey-icon"><MapPin size={20} /></span><div className="history-main"><div><h2>{journey.destination}</h2><StatusBadge status={journey.status} /></div><p>{journey.origin} → {journey.destination}</p><div className="history-meta"><span><CalendarDays size={15} />{formatDateTime(journey.startedAt, state.user.locale, state.user.dateFormat)}</span><span><Clock3 size={15} />{formatDuration(journey.durationMinutes, state.user.locale, { hours: t("common.hours"), minutes: t("common.minutes") })}</span><span>{t(`travel.${journey.travelType}` as Parameters<typeof t>[0])}</span></div><div className="history-signals">{journey.safetyCheckOccurred && <span><ShieldCheck size={14} />{t("journeys.safetyCheck")}</span>}{journey.prototypeEscalationTriggered && <span className="warning"><AlertTriangle size={14} />{t("journeys.prototypeEscalation")}</span>}</div></div><ChevronRight size={19} /></Link>)}</div>}
