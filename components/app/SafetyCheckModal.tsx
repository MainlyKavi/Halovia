...
export function SafetyCheckModal() {
  ...
  const formattedSeconds = formatNumber(seconds, state.user.locale);
  ...
  return createPortal(<div className="safety-screen" role="presentation">
    <div ref={panelRef} className="safety-panel" role="dialog" aria-modal="true" aria-labelledby="safety-title" aria-describedby="safety-description">
      <div className={`countdown-ring ${alerted ? "alerted" : ""}`} style={{ "--countdown": `${ratio}` } as React.CSSProperties}><span role="timer" aria-label={t("safety.secondsRemaining", { seconds: formattedSeconds })}>{alerted ? <AlertTriangle size={31} /> : formattedSeconds}</span></div>
      <h1 id="safety-title">{t("safety.title")}</h1>
      <p id="safety-description">{alerted ? t("safety.alerted") : t("safety.text")}</p>
      {!alerted && <strong className="countdown-label">{t("safety.countdown", { seconds: formattedSeconds })}</strong>}
      <span className="sr-only" aria-live="polite">{announce ? (alerted ? t("safety.alerted") : t("safety.countdown", { seconds: formattedSeconds })) : ""}</span>
