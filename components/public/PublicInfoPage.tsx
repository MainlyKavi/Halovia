...
import { AlertTriangle, ArrowLeft, Check, FileText, LockKeyhole, MessageSquareText, Send, ShieldAlert, WifiOff } from "lucide-react";
...
function FeedbackForm({ kind }: { kind: "feedback" | "report" }) {
  ...
  async function submit() {
    ...
    try {
      await haloviaApi.submitFeedback(kind === "report" ? "problem" : "feedback", text.trim());
      setStatus("submitted");
      setText("");
    } catch {
      setStatus("error");
